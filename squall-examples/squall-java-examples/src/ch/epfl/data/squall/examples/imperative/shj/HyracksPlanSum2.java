...
public class HyracksPlanSum2 extends QueryPlan {
	
	private static final NumericType<Double> _doubleConv = new DoubleType();

    public HyracksPlanSum2(String dataPath, String extension, Map conf) {
    ...
    public Component createQueryPlan(String dataPath, String extension, Map conf) {
		...
		// final AggregateOperator agg = new ApproximateSumSamplingOperator(new ColumnReference(_doubleConv, 0), conf, 1500000)
		//			     						.setGroupByColumns(Arrays.asList(1));
		
		final AggregateOperator agg = new AggregateSumOperator(new ColumnReference(_doubleConv, 0), conf)
				     						.setGroupByColumns(Arrays.asList(1));
